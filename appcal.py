...
if not st.session_state.authenticated:
    ...
    if choice == "Cerrar Sesión":
        ...
        if st.session_state.role == 'administrador':
            documents = user_management.get_all_documents()
        elif st.session_state.role == 'usuario':
            documents = user_management.get_documents_by_reviewer(st.session_state.username)
    
        if documents:
            for doc in documents:
                st.markdown(f"**Código del Documento**: {doc['doc_code']}")
                st.markdown(f"**Apellidos del Investigado**: {doc['last_name']}")
                st.markdown(f"**Nombre del Investigado**: {doc['first_name']}")
                st.markdown(f"**DNI del Investigado**: {doc['dni']}")
                st.markdown(f"**Encargado de Revisar el Documento**: {doc['reviewer']}")
                st.markdown(f"**Contenido del Documento**: {doc['content']}")
                st.markdown("---")
        else:
        ...
        if doc_action == "Agregar Documento":
            ...
            if 'edit_document' in st.session_state:
                ...
                with st.form("edit_document_form"):
                    doc_code = st.text_input("Código del Documento", value=document['codigo'])
                    last_name = st.text_input("Apellidos del Investigado", value=document['apellidos'])
                    first_name = st.text_input("Nombre del Investigado", value=document['nombre'])
                    dni = st.text_input("DNI del Investigado", value=document['dni'], max_chars=8)
                    ...
                    users = user_management.get_users()
                    normal_users = [user['username'] for user in users if user['role'] == 'usuario']
                    reviewer = st.selectbox("Encargado de Revisar el Documento", normal_users, index=normal_users.index(document['revisor']))

                    doc_content = st.text_area("Contenido del Documento", value=document['contenido'])
        ...
        if user_action == "Agregar Usuario":
            ...
            with st.form("add_user_form"):
                ...
                if add_user_button:
                    user_management.create_user(username, password, role)
            ...
            if 'edit_user' in st.session_state:
                ...
                with st.form("edit_user_form"):
                    ...
                    role = st.selectbox("Rol", ["administrador", "usuario"], index=["administrador", "usuario"].index(user['role']))
                    edit_user_button = st.form_submit_button("Actualizar Usuario")

                    if edit_user_button:
                        user_management.update_user_password(username, password)
                        user_management.update_user_role(username, role)
        ...
        if st.session_state.role == "usuario":
            ...
            with st.form("change_password_form"):
                ...
                change_password_button = st.form_submit_button("Actualizar Contraseña")
                    ...
                    if current_password and new_password and confirm_password:
                        ...
                        if user and user['password'] == current_password:
                            if new_password == confirm_password:
                                user_management.update_user_password(username, new_password)
                            ...
                            if st.session_state.password_attempts <= 0:
                                st.error("Demasiados intentos incorrectos. Cerrando sesión.")
                                st.session_state.authenticated = False
                                st.session_state.role = None
                                st.session_state.username = None
                    else:
...
# Al finalizar la ejecución de la aplicación, cerrar la conexión    
